...
class McNinja_Post_Transporter {
	...
	function settings_api_init() {
		...
		add_settings_field( self::$option_name_enabled, '<span id="infinite-transporter-options">' . __( 'To infinity and beyond', 'jetpack' ) . '</span>', array( $this, 'infinite_setting_html' ), 'reading' );
	...
	function infinite_setting_html() {
		...
		if ( self::get_settings()->footer_widgets || 'click' == self::get_settings()->requested_type ) {
			...
			echo '<label><input name="infinite_transporter" type="checkbox" value="1" ' . checked( 1, '' !== get_option( self::$option_name_enabled ), false ) . ' /> ' . __( 'Scroll Infinitely', 'jetpack' ) . '</br><small>' . sprintf( __( '(Shows %s posts on each load)', 'jetpack' ), number_format_i18n( self::get_settings()->posts_per_page ) ) . '</small>' . '</label>';
		}
	}
